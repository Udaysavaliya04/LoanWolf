...
function App() {
  ...
  async function fetchDashboard() {
    try {
      const res = await fetch(withBase('/api/loans/dashboard'), { credentials: 'include' });
...
async function fetchLoans() {
  ...
  const loadSchedule = async (loanId = selectedLoanId) => {
    if (!loanId) {
      setError('Select a loan first');
      return;
    }
    ...
    try {
      const res = await fetch(withBase(`/api/loans/${loanId}/schedule`), {
        credentials: 'include',
      });
      if (!res.ok) {
        const err = await res.json();
  ...
  const handleDeleteLoan = async (loanId) => {
    ...
    try {
      ...
      const remainingLoans = loans.filter((l) => l._id !== loanId);
      setLoans(remainingLoans);
      
      if (selectedLoanId === loanId) {
        setEvents([]);
        if (remainingLoans.length > 0) {
            setSelectedLoanId(remainingLoans[0]._id);
        } else {
            setSelectedLoanId(null);
        }
      }
      
      // Update dashboard after delete
      fetchDashboard();
    } catch (err) {
      console.error(err);
      setError(err.message);
  ...
  const handleSaveEvent = async (updatedEvent) => {
    try {
      const payload = {
        ...
        amount: updatedEvent.type === 'EXTRA_PAYMENT' ? Number(updatedEvent.amount) : undefined,
        newAnnualInterestRate: updatedEvent.type === 'RATE_CHANGE' ? Number(updatedEvent.newAnnualInterestRate) : undefined,
        note: updatedEvent.note || undefined,
      };

      const res = await fetch(withBase(`/api/loans/${selectedLoanId}/events/${updatedEvent._id}`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
        credentials: 'include',
        ...
        <div className="site-header-inner">
          ...
          <div className="desktop-nav">
            ...
            <div className="site-header-user">
              ...
              >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16.5" height="16.5" fill="none" stroke="#000000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 1 }}><path d="m14.305 19.53l.923-.382m0-2.296l-.923-.383m2.547-1.241l-.383-.923m.383 6.467l-.383.924m2.679-6.468l.383-.923m-.001 7.391l-.382-.924M2 21a8 8 0 0 1 10.434-7.62m8.338 3.472l.924-.383m-.924 2.679l.924.383"/><circle cx="10" cy="8" r="5"/><circle cx="18" cy="18" r="3"/></svg>             
              </button>
              <button type="button" className="secondary-btn site-header-logout" style={{background:'linear-gradient(180deg, #d90000ff 0%, #ff0000ff 100%)', color:'white', height:'30.8px'}} onClick={handleLogout}>
                Log out
              </button>
            </div>
          </div>
          ...
          <div className="mobile-actions" style={{ gap: '0.2rem', alignItems: 'center' }}>
            <button
              ...
              >
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16.5" height="16.5" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 1 }}><path d="m14.305 19.53l.923-.382m0-2.296l-.923-.383m2.547-1.241l-.383-.923m.383 6.467l-.383.924m2.679-6.468l.383-.923m-.001 7.391l-.382-.924M2 21a8 8 0 0 1 10.434-7.62m8.338 3.472l.924-.383m-.924 2.679l.924.383"/><circle cx="10" cy="8" r="5"/><circle cx="18" cy="18" r="3"/></svg>  
            </button>
            <div
        ...
        <div className={`mobile-menu-overlay ${mobileMenuOpen ? 'open' : ''}`}>
          ...
          <div className="site-header-user">
            ...
            <button
              ...
              style={{background:'linear-gradient(180deg, #d90000ff 0%, #ff0000ff 100%)', color:'white'}}
              onClick={() => {
                handleLogout();
                setMobileMenuOpen(false);
              }}
          ...
          {loans.length === 0 ? (
            ...
            <>
                ...
                <div className="loan-summary">
                  ...
                  <div className="loan-actions" style={{ gridColumn: 'span 2', justifyContent: 'flex-start' }}>
                     ...
                     <button className="action-btn delete" onClick={() => handleDeleteLoan(currentLoan._id)} title="Delete Loan">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path fill="none" d="M10 11v6m4-6v6m5-11v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                       Delete Loan
                     </button>
                  </div>
                </div>
              ...
              <form className="form" onSubmit={submitEvent}>
                ...
                <button type="submit" className="secondary-btn" style={{height:'35px'}}>
                  Save event
                </button>
              </form>
                        ...
                        <tr key={ev._id}>
                          ...
                          <td className="events-actions-cell">
                            <div className="action-btn-group events-action-btn-group">
                              <button className="action-btn edit" onClick={() => setEditingEvent(ev)} title="Edit Event" style={{ marginRight: '0.4rem' }}>
